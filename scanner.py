...
USER_AGENT = "Mozilla/5.0 (Linux; Android 13; SM-S918N) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/131.0.0.0 Mobile Safari/537.36"
...
class NaverCrawler:
    ...
    def extract_articles(self, html, source_url, source_name):
        ...
        for a in soup.find_all("a", href=True):
            ...
            # 제목 추출 - 다양한 위치 체크
            title = ""
            # 1) <strong>, <span> 내부 텍스트
            for selector in ["strong", "span.text", "span.title", "em"]:
                elem = a.select_one(selector)
                if elem:
        ...
        return articles[:50]  # 최대 50개 (랭킹 30위까지 + 여유)
...
# ─────────────────────────────────────────────
# 앵글 추출기 (기사에서 파생 주제 시드 추출)
# ─────────────────────────────────────────────
class AngleExtractor:
    """기사 제목에서 다양한 앵글(시드) 추출"""
    ...
    NICKNAME_PATTERN = re.compile(r"['\"]([^'\"]{2,10})['\"].*(?:배우|가수|아이돌|스타|감독|선수)")
    ...
    def generate(self, angle):
        ...
        if ages:
            ...
            topics.append({
                ...
                "angle_type": "나이",
                "seed": f"{main_person} {age}세 언급" if main_person else f"{age}세 관련",
                "template": "동갑내기",
            })
            topics.append({
        ...
        if "질병·건강" in tags:
            topics.append({
                ...
                "angle_type": "질병·복귀",
                "seed": f"{main_person} 완치" if main_person else "완치 언급",
                "template": "투병 극복",
            })
            topics.append({
        ...
        if "공백·복귀" in tags and "질병·건강" not in tags:
            topics.append({
                ...
                "angle_type": "복귀",
                "seed": f"{main_person} 복귀" if main_person else "복귀 언급",
                "template": "장기 공백 복귀",
            })
        ...
        if "논란·폭로" in tags:
            ...
            topics.append({
                ...
                "angle_type": "논란",
                "seed": f"{main_person} {scandal_type}" if main_person else f"{scandal_type} 이슈",
                "template": "논란 집합",
            })
        ...
        if "결혼·이혼" in tags:
            if "이혼" in title or "재혼" in title:
                topics.append({
                    ...
                    "angle_type": "결혼·이혼",
                    "seed": f"{main_person} 이혼" if main_person else "이혼 후 근황",
                    "template": "이혼 반전",
                })
                ...
                topics.append({
                    ...
                    "angle_type": "결혼·이혼",
                    "seed": f"{main_person} 열애" if main_person else "열애 이슈",
                    "template": "열애 결별",
                })
        ...
        if "스포츠" in tags:
            ...
            if "FA" in sport_kw or "연봉" in sport_kw or "MLB" in sport_kw:
                topics.append({
                    ...
                    "angle_type": "스포츠·몸값",
                    "seed": f"{main_person} {sport_kw}" if main_person else sport_kw,
                    "template": "선수 몸값",
                })
...
# ─────────────────────────────────────────────
# 표 데이터 생성 (캔바 붙여넣기용 TSV)
# ─────────────────────────────────────────────
def generate_table_template(topic):
    """주제별 TSV 표 템플릿 생성 (10행 빈칸)"""
...
def run_scan():
    ...
    for src in SOURCES:
        print(f"\n📡 {src['name']} 크롤링...")
        html = crawler.fetch(src["url"])
        if not html:
            source_stats[src["name"]] = 0
            continue
        
        articles = crawler.extract_articles(html, src["url"], src["name"])
        for a in articles:
    ...
    # 4) TOP 주제 선별 (점수 6.0 이상)
    top_topics = [t for t in valid_topics if t["score"] >= 6.0][:30]
    print(f"  최종 추천: {len(top_topics)}개 (점수 6.0 이상)")
    ...
    result = {
        ...
        "topics": top_topics,
        "all_topics": valid_topics,  # 전체 (UI 필터용)
    }
    ...
    print(f"\n{'='*60}")
    print(f"🎯 TOP 5 추천 주제 미리보기")
    print(f"{'='*60}")
    for i, t in enumerate(top_topics[:5], 1):
        print(f"  {i}. [점수 {t['score']}] {t['title']}")
        print(f"     원본: {t['source_article'][:50]}")
        print(f"     중복체크: {t['dup_check']['status']}")
